...
| `02_Base/` | **完整** — EventBus、ServiceLocator、StateMachine、Timer、ResourceManager、ObjectPool、CommandSystem、GameState、Audio、UI（UIPanel/UIWindow/WindowManager）、VFX |
...
| `05_Show/` | Loading、MainMenu、Notification、PauseMenu、Settings |
...
- UI窗口事件：`02_Base/EventBus/Events/WindowEvents.cs`（WindowManager 发布，WindowOpened/Closed/Focused/AllClosed）

所有事件必须为 `struct` 并实现 `IEvent` 接口。

#### ServiceLocator（`02_Base/ServiceLocator/ServiceLocator.cs`）
...
- **TimerSystem**（`MonoSingleton`）：零GC定时器，通过 `TimerHandle` 安全取消。优先使用此系统而非 Unity 的 `Invoke`
  ...
  ```
- **ResourceManager**（`MonoSingleton`）：同步/异步加载、AssetBundle、LRU缓存、3次重试。拆分为 3 个 partial 文件：`ResourceManager.cs`（核心+IResourceLoader）、`ResourceManager.Bundle.cs`（IAssetBundleLoader）、`ResourceManager.Utilities.cs`（事件/验证/工具/批量/调试）
...
- **VFXManager**（`MonoSingleton`，位于 `02_Base/VFX/VFXManager.cs`）：视觉特效管理，通过 `VFXEntry[]` 配置
...
Awake:
  Phase 1: 创建 MonoSingletons（无数据依赖）
    ...
    → ResourceManager → AudioManager → VFXManager
...
- **UIPanel**（`MonoBehaviour`，`02_Base/UI/`）：场景预置面板基类（需 `CanvasGroup`）。面板默认隐藏（Awake 中自动设置），由各自的 Presenter 通过 `Show()` / `Hide()` 直接控制。
- **WindowManager**（`MonoSingleton`，`02_Base/UI/`）：管理动态加载的 UIWindow（纯 C# 类），通过 JSON 配置驱动，支持 DOTween 入场/退场动画。窗口类在 `WindowClassRegistry` 中注册。
...
| 新增 UI 面板（场景预置） | 继承 `UIPanel`，实现 Presenter/ViewModel/View | 3-4个文件 |
| 新增 UI 窗口（动态加载） | 继承 `UIWindow`，增加 JSON 配置项，WindowClassRegistry 注册 | 3-4个文件 |
